...
if(pred_acc > 0.20):
    call = "python3 subgf_a2d.py " + str(ID) + " " + str(pred_acc)
    call_str = "sshpass -p 'umbrella' ssh -o StrictHostKeyChecking=no pranshu@10.237.20.124 '" + call +"'"
    subprocess.getoutput(call_str)
    
call = "python3 grank_a2d.py " + str(ID)
call_str = "sshpass -p 'umbrella' ssh -o StrictHostKeyChecking=no pranshu@10.237.20.124 '" + call +"'"
...
'''   
comment("Grade for part (d) (tentative and based on best submission till now) = " + str(pred_acc));
